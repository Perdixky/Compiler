...
template <typename... Args> struct fatal_rformat {
  ...
  consteval fatal_rformat(const StrLike& str, std::source_location location = std::source_location::current())
...
export template <typename... Args> [[noreturn]] void fatal(fatal_format<Args...> fmt, Args&&... args) {
  try {
    ...
    std::println(stderr, "fatal: {}\n  at {}:{}:{}\n  in {}", message, fmt.location.file_name(), fmt.location.line(),
                 fmt.location.column(), fmt.location.function_name());
  } catch (const std::exception& e) {
    std::println(stderr, "fatal: <failed to format fatal message: {}>\n  at {}:{}:{}\n  in {}", e.what(),
                 fmt.location.file_name(), fmt.location.line(), fmt.location.column(), fmt.location.function_name());
  } catch (...) {
    std::println(stderr, "fatal: <failed to format fatal message>\n  at {}:{}:{}\n  in {}", fmt.location.file_name(),
                 fmt.location.line(), fmt.location.column(), fmt.location.function_name());
  }
